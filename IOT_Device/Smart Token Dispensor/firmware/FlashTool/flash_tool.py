...
prefix (default "TOKEN_DISPENSER", set at register-model time via --vps-env-prefix)
...
DEFAULT_VPS_ENV_PREFIX = "TOKEN_DISPENSER"
DEFAULT_VPS_DIR = "/root/secrets/iot-one/token-dispenser-factory-records"
...
def get_source_info(project_dir):
    ...
    status = subprocess.run(["git", "-C", project_dir, "status", "--porcelain",
                             "--untracked-files=all", "--", "."],
                             capture_output=True, text=True)
    # A source tree that was copied/renamed but never committed can otherwise
    # inherit the parent repository's HEAD and falsely claim that commit as the
    # flashed source. Treat untracked content as dirty too. Generated records,
    # caches and logs belong in .gitignore, not in a production override.
    dirty_files = []
    untracked_files = []
    ...
    return {
        ...
        "dirty": len(dirty_files) > 0 or len(untracked_files) > 0,
...
def run_factory_flash(model_id, port=None, skip_erase=False, force=False,
    ...
    if source_info["commit"] is None:
        ...
        if source_info["dirty"]:
            log(f"WARNING: source tree has {len(source_info['dirty_files'])} modified "
                f"tracked and {len(source_info['untracked_files'])} untracked file(s); "
                "this build will NOT match that commit:")
            for f in source_info["dirty_files"][:10]:
                log(f"  modified: {f}")
            for f in source_info["untracked_files"][:10]:
                log(f"  untracked: {f}")
            if not allow_dirty:
                raise FlashError(
                    "Refusing to flash from modified or untracked source - the "
    ...
    # (Smart Token Dispenser's own concept) alongside the shared
    ...
    # Installer-friendly file containing only the BLE setup identity and PoP.
    # Keep the more privileged local API token exclusively in the JSON factory
    # record instead of duplicating it into a handoff text file.
    pop_path = os.path.join(RECORDS_DIR, f"{safe_name}_{ts}_POP.txt")
    with open(pop_path, "w", encoding="utf-8") as f:
        f.write(f"Product ID: {record['pid']}\n")
        f.write(f"BLE name: {record['ble_name']}\n")
        f.write(f"PoP username: {record['username']}\n")
        f.write(f"PoP: {record['pop']}\n")
    log(f"Saved local PoP text file: {pop_path}")
    factory_record["pop_text_path"] = pop_path
...
def upload_to_vps(record_path, model, log=print):
    ...
    (model["vps_env_prefix"], default "TOKEN_DISPENSER" for models registered
