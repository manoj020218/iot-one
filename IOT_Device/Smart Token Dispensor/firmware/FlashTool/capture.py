...
# from a previous boot (unlike the product-specific patterns above, which
...
FACTORY_RE = re.compile(
    ...
    r"(?: api_header=(?P<api_header>\S+) api_token=(?P<token>\S+))?"
)

# Smart Token Dispenser's own local HTTP API auth token concept -- not
...
def capture_boot_record(port, baud=115200, timeout_s=60, required_fields=DEFAULT_REQUIRED_FIELDS):
    ...
    stack bring-up itself takes another ~20-30s wall-clock. The capture exits
    as soon as all required fields arrive, so the longer ceiling does not slow
    a normal boot.
    """
    record = {}
    raw_lines = []
